...
matrix_H = np.loadtxt('matrix_H.txt')
matrix_H_mean = np.loadtxt('matrix_H_mean.txt')
...
for i in range(256):
    ...
    for j in range(samplepoints):
        ...
        for l in range(number_of_traces):
            left = matrix_H[l][i] - matrix_H_mean[i]
            right = matrix_P[l][j] - matrix_P_mean[j]

            cov += left*right  
            divisor_left += left*left
            divisor_right += right*right
