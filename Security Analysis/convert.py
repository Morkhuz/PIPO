#imports
import numbers
from pydoc import plain
import numpy as np
import math
import matplotlib.pyplot as plt


Sbox_values = []
sbox = np.array(Sbox_values)

def convert(X):
    T = np.array([0, 0, 0, 0, 0, 0, 0, 0])

    for i in range(8):
        for j in range(8):
            T[i] |= (((X[j] & (1<<i))>>i) << j)
    
    for i in range(8):
        X[i] = T[i]


def sbox_TLU(X, k):
    X[0] = X[0] ^ k
    print('X xor k = ' + hex(X[0]))
    convert(X)
    print ('X after 1. convert: ' + hex(X[0])[2:] + ' ' + hex(X[1])[2:] + ' ' + hex(X[2])[2:] + ' ' + hex(X[3])[2:] + ' ' + hex(X[4])[2:] + ' ' + hex(X[5])[2:] + ' ' + hex(X[6])[2:] + ' ' + hex(X[7])[2:])
    for i in range(8):
        X[i] = sbox[X[i]]
        print('X[' + str(i) + '] = sbox[X[' + str(i) + ']] = ' + hex(X[i])[2:])
    convert(X)
    print ('X after 2. convert: ' + hex(X[0])[2:] + ' ' + hex(X[1])[2:] + ' ' + hex(X[2])[2:] + ' ' + hex(X[3])[2:] + ' ' + hex(X[4])[2:] + ' ' + hex(X[5])[2:] + ' ' + hex(X[6])[2:] + ' ' + hex(X[7])[2:])
    return X



X = np.array([0x26, 0x00, 0x27, 0x1E, 0xF6, 0x52, 0x85, 0x09])
k = 0x97


output = sbox_TLU(X, k)
print(hex(output[0]))

